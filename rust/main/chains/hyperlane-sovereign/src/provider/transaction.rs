...
impl SovereignClient {
    ...
    async fn sign_tx(&self, mut utx_json: Value) -> ChainResult<Value> {
        ...
        if env::var("SOV_TEST_UTILS_FIXED_CHAIN_HASH").is_ok() {
            ...
            // TODO: prehash chain hash on client construction and use `.cached_chain_hash()`
            // instead, when the `Schema` becomes `Send`
            let chain_hash = schema
                .chain_hash()
                .map_err(|e| custom_err!("Couldn't compute chain hash: {e}"))?;
